...
execute as @e[type=marker,tag=markerfourlb,sort=nearest,limit=1] run scoreboard players set @s distance_lb 50
execute as @e[type=marker,tag=markerfourlb,sort=nearest,limit=1] run function neifours:magic/magicstick/lb/markerforlb
#後ろ処理
#tagを消すだけ
#あと必要MPもここから変えてね！

...
execute as @s[tag=magic_pot_counter] if score @s mp_items matches 5.. run tag @s add magic_power
...
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run scoreboard players set @s tp_distacne 100
...
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run effect give @s speed 32 0
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run effect give @s regeneration 10 0
#後ろ処理
#tagを消すだけ
#あと必要MPもここから変えてね！
execute as @s[tag=magic_power] run clear @s splash_potion[custom_data={MagicPot:true}] 5

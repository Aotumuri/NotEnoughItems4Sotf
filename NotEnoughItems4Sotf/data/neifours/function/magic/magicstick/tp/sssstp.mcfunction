...
execute as @s[tag=magic_pot_counter] if score @s mp_items matches 10.. run tag @s add magic_power
...
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run scoreboard players set @s tp_distacne 200
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run tag @s add tag_tp_tp
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run function neifours:magic/magicstick/tp/markerforsssstp
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run effect give @s speed 32 2
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run effect give @s regeneration 10 2
execute as @e[type=marker,tag=markerfortp,sort=nearest,limit=1] run effect give @s resistance 5 9
#後ろ処理
#tagを消すだけ
#あと必要MPもここから変えてね！
execute as @s[tag=magic_power] run clear @s splash_potion[custom_data={MagicPot:true}] 10
